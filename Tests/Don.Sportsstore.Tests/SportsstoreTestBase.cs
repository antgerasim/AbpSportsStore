...
namespace Don.Sportsstore.Tests
{
    public abstract class SportsstoreTestBase : AbpIntegratedTestBase<SportsstoreTestModule>
    {
        ...
        protected void UsingDbContext(Action<SportsstoreDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SportsstoreDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SportsstoreDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SportsstoreDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SportsstoreDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SportsstoreDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SportsstoreDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SportsstoreDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SportsstoreDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SportsstoreDbContext, Task<T>> func)
        {
            T result;
